...
// JSX específico para o CharacterInGame
interface CharacterInGameProps {
  type: string;
  color: string;
  username?: string;
  health?: number;
...
export default function DemoGameBoard() {
  ...
  // Curar um jogador
  const healPlayer = (playerId: string, amount: number) => {
    setPlayers(players.map(player => {
      if (player.id === playerId) {
        return {
          ...player,
  ...
  // Função vazia para callback
  const emptyFunction = () => {};
  
  return (
    <div className="min-h-screen bg-background text-foreground py-8">
      <div className="container px-4 mx-auto">
        <h1 className="text-3xl font-bold text-center mb-8">Tabuleiro de Jogo</h1>
              ...
              >
                ...
                {players.map(player => (
                  <CharacterInGame
                    ...
                    onActionComplete={player.id === currentPlayer.id ? handleActionComplete : emptyFunction}
                  />
                ))}
              </div>
            </div>
              ...
              <div className="flex items-center space-x-4">
                <div className="w-16 h-16">
                  <CharacterInGame
                    ...
                    onActionComplete={emptyFunction}
                  />
                </div>
                <div>
