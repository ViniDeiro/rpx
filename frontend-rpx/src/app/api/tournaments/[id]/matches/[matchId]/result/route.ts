...
async function isMatchParticipant(tournament: any, matchId: string, userId: string) {
  const match = tournament.matches.find((m: any) => m._id.toString() === matchId);
  
  if (!match) return false;
  
  const isParticipant1 = match.participant1Id && match.participant1Id.toString() === userId;
  const isParticipant2 = match.participant2Id && match.participant2Id.toString() === userId;
  
  return isParticipant1 || isParticipant2;
}

// POST: Reportar resultado de uma partida específica
export async function POST(
  ...
  try {
    ...
    const matchIndex = tournament.matches.findIndex((m: any) => m._id.toString() === matchId);
    ...
    const isWinnerParticipant = 
      match.participant1Id.toString() === winnerId ||
      match.participant2Id.toString() === winnerId;
    ...
    const loserId = match.participant1Id.toString() === winnerId 
      ? match.participant2Id.toString()
      : match.participant1Id.toString();
    ...
    if (match.nextMatchId) {
      const nextMatchIndex = tournament.matches.findIndex(
        (m: any) => m.bracketPosition === match.nextMatchId
      );
    ...
    if (match.nextLoseMatchId) {
      const nextLoseMatchIndex = tournament.matches.findIndex(
        (m: any) => m.bracketPosition === match.nextLoseMatchId
      );
    ...
    const allMatchesCompleted = tournament.matches.every((m: any) => 
