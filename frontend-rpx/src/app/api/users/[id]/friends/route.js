  ...
  try {
    ...
    if (action === 'favorite' || action === 'unfavorite') {
      ...
      await User.updateOne(
        { 
          _id: currentUserId,
          'friends.userId': mongoose.Types.ObjectId(friendId)
        },
        {
      ...
      await User.findByIdAndUpdate(
        currentUserId,
        { $pull: { friends: { userId: mongoose.Types.ObjectId(friendId) } } }
      );
      
      // Remover da lista de amigos do outro usuário
      await User.findByIdAndUpdate(
        friendId,
        { $pull: { friends: { userId: mongoose.Types.ObjectId(currentUserId) } } }
      );
      ...
      await User.findByIdAndUpdate(
        currentUserId,
        { $pull: { blockedUsers: { userId: mongoose.Types.ObjectId(friendId) } } }
      );
