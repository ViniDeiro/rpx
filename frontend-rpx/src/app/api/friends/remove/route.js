...
export async function POST(request) {
  try {
    ...
    if (!friend) {
      return NextResponse.json({ error: 'Usuário amigo não encontrado' }, { status: 400 });
    }
    ...
    if (!areFriends) {
      return NextResponse.json({ error: 'Este usuário não está na sua lista de amigos' }, { status: 400 });
    }
